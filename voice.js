// VoiceRSS Javascript SDK adapted
...
// Voice API to use VoiceRSS or Speech Synthesiser
const VoiceAPI = {
  speechLang: 'en-us', // 'fr-fr',
  voiceRSSParams: (joke, lang) => new {
    //key: '1404...0768',
    hl: lang,
    src: joke,
    r: 0,
  ...
  localSpeech: function (joke, lang) {
    const toSpeech = new SpeechSynthesisUtterance();
    toSpeech.text = joke;
    toSpeech.lang = lang;
    toSpeech.onend = event => {
      document.dispatchEvent(new Event('speechSynthesizerEnded'));
    }
    const voices = window.speechSynthesis.getVoices();
    if (voices.length > 0) {
      console.log('Availabel voices', voices);
    }
    window.speechSynthesis.speak(toSpeech);
  },
  voiceRSSspeech: function (joke) {
    VoiceRSS.speech(voiceRSSParams(joke, this.speechLang));
  },
  tellMe: function (joke) {
    if (this.voiceRSSParams.key) {
      this.voiceRSSspeech(joke);
    } else if (this.localSpeech) {
      this.localSpeech(joke, this.speechLang);
    } else {
